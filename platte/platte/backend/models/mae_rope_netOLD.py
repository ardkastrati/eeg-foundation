...
class MaskedAutoencoderViTRoPE(nn.Module):
    ...
    def forward_encoder(self, x, means, stds, channels, win_size, mask_ratio):
        """ """
        B, C, H, W = x.shape
    ...
    def forward_decoder(self, x, nr_meta_patches, H, W, win_size):
        ...
        # print("[forward_decoder] after nr_meta_patches removal:", pred.shape, "?")
    ...
    def forward(self, batch):
        ...
        x_emb, meta_patches, mask, nr_meta_patches = self.forward_encoder(
            x=x,
            means=means,
            stds=stds,
            channels=channels,
    ...
    def initialize_weights(self):
        ...
        # --------------------------------------------------------------------------
        # Class Token and Channel Tokens Initialization

        # timm's trunc_normal_(std=.02) is effectively normal_(std=0.02) as cutoff is too big (2.)
        torch.nn.init.normal_(self.cls_token, std=0.02)

        for wrapper in self.channel_encoding_map.values():
