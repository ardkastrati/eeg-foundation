...
@st.cache
def detect_abnormal(model_name, data):
...
def app():
    st.markdown("## Abnormal Activity Detection")
    ...
    if uploaded_image:
        ...
        with button_col1:
            if st.button("Detect Abnormal Activity"):
                with st.spinner("Computing probability..."):
                    abnormal_probability = float(detect_abnormal(model, original_image))
                    st.session_state.abnormal_probability = abnormal_probability

        with button_col2:
            if st.button("Compute Attention"):
                abnormal_attention_weights = get_attention(model, original_image)
                st.session_state.abnormal_attention_weights = abnormal_attention_weights

        with button_col3:
            option = st.selectbox("Choose the channel to plot:", tuple(channels))
            if st.button("Plot Brain Activity"):
                st.session_state.abnormal_channel_image = "/Users/ardkastrati/Documents/YC/platte/Demo/PatientX/test.png"
                st.session_state.abnormal_channel_image = option

        col0, col1, col2 = st.columns([1, 2, 3])

        if 'abnormal_probability' in st.session_state:
            col0.markdown("### Abnormal Probability")
            col0.pyplot(plot_probability(st.session_state.abnormal_probability))

        if 'abnormal_attention_weights' in st.session_state:
            col1.markdown("### Attention Map")
            col1.pyplot(plot_topomap(st.session_state.abnormal_attention_weights, channels))

        if 'abnormal_channel_image' in st.session_state:
            channel = Image.open(st.session_state.abnormal_channel_image)
            col2.markdown("### Patient's Brain Activity: " + st.session_state.abnormal_channel_image)
